                    ...
                    <div class="entry-content">
                        <?php the_content(); ?>
                                <?php
                                    ...
                                    // JSON с картой перелинковки
                                    $json_path = get_template_directory() . '/data/internal_links_related_casinos_clean.json';
                                    $map_json  = is_readable($json_path) ? file_get_contents($json_path) : '';
                                    $map       = $map_json ? json_decode($map_json, true) : [];

                                    // Текущий URL/slug
                                    $current_url  = trailingslashit( home_url( add_query_arg([], $wp->request) ) );
                                    $path_parts   = array_values(array_filter(explode('/', wp_parse_url($current_url, PHP_URL_PATH))));
                                    $current_slug = end($path_parts);

                                    // Ищем по полному URL, затем по slug
                                    $items = $map[$current_url] ?? [];
                                    if (!$items && $map) {
                                    foreach (array_keys($map) as $key_url) {
                                        $key_parts = array_values(array_filter(explode('/', wp_parse_url($key_url, PHP_URL_PATH))));
                                        $key_slug  = end($key_parts);
                                        if ($key_slug === $current_slug) { $items = $map[$key_url]; break; }
                                    }
                                    }

                                    // Локализованные тексты — сначала через Polylang, иначе через gettext
                                    $title = function_exists('pll__') ? pll__('Related casinos') : __('Related casinos', 'your-textdomain');
                                    $desc  = function_exists('pll__') ? pll__('Here are other casinos you might like.') : __('Here are other casinos you might like.', 'your-textdomain');

                                    // Разрешаем URI цели на текущем языке
                                    $resolve_target_url = function (string $url): string {
                                    ...
                                    <?php if (!empty($items)) : ?>
                                    <section class="related-casinos" aria-labelledby="related-casinos-title">
                                        <h2 id="related-casinos-title"><?= esc_html($title); ?></h2>
                                        <p><?= esc_html($desc); ?></p>
                                        <ul class="related-casinos__list">
                                        <?php foreach ($items as $it):
                                            $target_url = $resolve_target_url($it['target_url']);
                                            $anchor     = $it['anchor_text'] ?? '';
                                        ?>
                                            <li class="related-casinos__item">
                                            <a class="related-casinos__link" href="<?= esc_url($target_url); ?>">
                                                <?= esc_html($anchor); ?>
                                            </a>
                                            </li>
                                        <?php endforeach; ?>
